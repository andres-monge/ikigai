...
/** Represents a single part of a prompt or response. */
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!jsonResponse.candidates || jsonResponse.candidates.length === 0) {
        // Sometimes the API returns a response with no candidates if content is blocked.
      ...
      if (attempt === maxRetries) {
        // Re-throw the last error if all retries fail
        throw error;
      }
...
export async function generateContentWithSearch(
  ...
  // Use the 'facts' model specifically for search-grounded queries
