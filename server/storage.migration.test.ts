...
async function assertMigratedShape(harness: Pick<StorageTestDatabaseHarness, 'pool' | 'database'>) {
  ...
  expect((await harness.pool.query(`
    select to_regclass('public.career_map_research_attempts') as attempts,
           to_regclass('public.career_map_evidence_associations') as associations
  `)).rows).toEqual([{ attempts: null, associations: null }]);
  expect((await harness.pool.query(`
    select
