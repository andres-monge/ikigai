...
- KTD4. **Conversation transcript lives in OpenAI Conversations, with server-side compaction.** Each user gets one OpenAI conversation object; turns pass its id through first-class `@ai-sdk/openai` Responses provider options (`conversation`, `store: true`), so OpenAI persists and threads the transcript. The current career-map briefing is request-scoped: send it on every model step as `providerOptions.openai.instructions`, and leave `ToolLoopAgent`'s top-level `instructions` unset because the U1 live spike proved that agent-level instructions become stored developer/system conversation items. Enable Responses-API compaction through `providerOptions.openai.contextManagement` with `compactThreshold: 1000` initially; the live API accepted that threshold and emitted a compaction item. Apply compaction only on step 0 of a new turn, after the prior tool loop has completed, and omit it from later tool-result continuation steps: forcing compaction while a client function-tool call was pending produced `400 No tool call found for function call output` in the U1 spike. Current provider documentation does not specify that plain user/assistant messages are excluded from compaction, so no product behavior may rely on that former caveat; the exact content selection remains provider-controlled and unverified. Chat history for the UI is read back via paginated `GET /v1/conversations/{id}/items` through a thin adapter to UI messages (U6). Deleting a conversation does not cascade to its items; erasure must paginate and delete every item before deleting the conversation itself (U9). (session-settled: user-directed — the user chose OpenAI-side transcript storage and explicitly wants compaction: "openai's compaction is great, we definitely want to make use of it"; chosen over BYO `UIMessage[]` persistence in Postgres: no transcript-persistence code to build and compaction comes free; accepted tradeoff: the transcript is readable only through OpenAI's API and couples that surface to the vendor — the career map, the durable product asset, stays in our database per KTD5.) Sources: OpenAI Conversations, Responses, and compaction documentation at developers.openai.com, plus the U1 live provider spike; verified 2026-08-17.
...
- **Verification:** `npm run check`, `npm test`, `npm run test:e2e`, `npm run build`; live provider spike output recorded in the implementation handoff (or PR, if one is used); a Vercel preview confirms `process.version` is v24.
...
- **Files:** server/ai/agent-prompts.ts (new module — the legacy server/ai/prompts.ts stays untouched: the legacy streaming chain imports its functions live, so rewriting it would break the protected questionnaire flow); golden-transcript scripts recorded with the implementation handoff (or PR, if one is used).
...
- **Verification:** Golden-transcript pass recorded in the implementation handoff (or PR, if one is used); `npm run check` (scratch script only).
...
- **Verification:** Golden-transcript pass recorded in the implementation handoff (or PR, if one is used); `npm run check`, `npm test`.
...
Golden transcripts are deliberately manual: the origin defers formal evals and rubrics until pilot evidence shows what is worth measuring (Scope Boundaries), and the repo rule requires asking before new production dependencies — so no eval framework is added in this build. Each transcript is a short script (opening message, objection, expected behavior, and the expected career-map state after each turn — reply behavior alone does not prove R18's writes were issued) checked against a dev conversation and recorded in the implementation handoff or, when one is used, the PR description.
...
- **Provider-options drift (Conversations, compaction).** These API surfaces are new and move; the U1 live spike proved the pinned SDK's first-class conversation, request-instructions, compaction, and abort options. U5 verifies pass-through at build time and keeps compaction on step 0 only. If a later API shape differs materially from the amended KTD4 facts, stop and surface (Goal Capsule).
- **Compaction expectation.** The provider controls which prior content is retained verbatim or represented through a compaction item; current documentation does not support the former claim that plain user/assistant messages are excluded. Long-lived pilots could still grow large transcripts; acceptable for the pilot horizon, revisit at Step 3 if turns slow down. U8 tests the product invariant that the current career-map briefing outranks stale transcript content after safe-boundary compaction, without asserting provider-internal item selection.
